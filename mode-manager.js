...
class ModeManager {
	...
	registerMode(modeName, handlers) {
		...
		console.log(`📌 Modo "${modeName}" registrado com ${Object.keys(handlers || {}).length} handlers`);
	}
	...
	setMode(modeName) {
		if (!this.handlers[modeName]) {
			throw new Error(`Modo não registrado: ${modeName}. Modos disponíveis: ${Object.keys(this.handlers).join(', ')}`);
		}
